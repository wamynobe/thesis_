...
the speaker-disjoint learner partitions used for evaluation. This chapter
evaluates how well those systems reproduce expert perceptual tone labels
...
\end{equation}
Accuracy remains important because it measures the overall rate of agreement
with expert labels. Macro-F1 is reported alongside accuracy because it exposes
whether that overall agreement hides poor behaviour on sparse categories such as
\(T_3\) or \(T_{\mathrm{NS}}\).
...
an independent speaker-disjoint split with $N=2{,}407$ test items and 487
validation items, selecting its checkpoint by validation accuracy. The five
SSL setups share their own speaker-disjoint learner split with $N=2{,}282$
test utterances and 464 development utterances; their selection metric is
learner-development accuracy for Setups~2, 3a--3c (learner stage), 4, and 5,
and learner-development macro-F1 for Setup~6. For the intermediate stage in
Setups~3a--3c the Stage~A checkpoint is picked on the AISHELL-1 development
set (693 utterances, frame accuracy) before the learner stage takes over.
...
speaker-disjoint learner test split. The CNN baseline is included as a
ToneNet-style reference; it uses its own speaker-disjoint split with a
slightly different speaker partition and is therefore not a fully matched
head-to-head comparison.

\begin{table}[H]
\centering
\caption[Aggregate test performance for the six experimental setups.]
{Aggregate test performance for the six experimental setups. SSL rows
share the speaker-disjoint learner test set ($N=2\,282$); the CNN row uses an
independent speaker-disjoint split ($N=2\,407$) and is reported as a
ToneNet-style reference. Higher values are better; best values per metric are
in \textbf{bold}.}
...
\midrule
1  & ToneNet-style CNN                                & n/a                          & 69.42 & 57.32 & 68.24 \\
...
Setup~5 leads in both partitions, and every dev/test gap is within roughly
two percentage points, indicating that checkpoint selection was stable; the
order between the closely spaced Wav2Vec~2.0 routes does shift between dev
and test, consistent with the narrow margin separating them.
...
Section~\ref{sec:exp:class_level}.
...
epoch. The confusion matrix (Figure~\ref{fig:exp:cnn_cm}) shows that the
residual error is concentrated on \(T_3\) and \(T_{\mathrm{NS}}\): \(T_3\) is
heavily confused with \(T_2\), and \(T_{\mathrm{NS}}\) productions are
scattered across all four canonical tones, consistent with the weak \(T_3\)
and \(T_{\mathrm{NS}}\) CNN scores in Tables~\ref{tab:exp:aggregate}
and~\ref{tab:exp:class_level}.
...
\begin{table}[H]
\centering
\caption[Per-class precision, recall, and \(F_1\) on the learner test split.]
{Per-class precision (P), recall (R), and \(F_1\) on the learner test split.
SSL systems share the speaker-disjoint learner test split ($N=2\,282$); the
CNN baseline uses its own speaker-disjoint split ($N=2\,407$). Values are in
percentage points; the strongest value for each class--metric pair (excluding
the CNN reference) is in \textbf{bold}. Confusion matrices for every setup
are reproduced in Appendix~\ref{sec:app:confusion_matrices}.}
...
\midrule
1  & CNN baseline                       & 43.84 & 33.84 & 38.21 & 71.16 & 80.06 & 75.33 & 71.48 & 73.33 & 72.39 & 32.81 & 16.41 & 21.88 & 77.72 & 79.92 & 78.80 \\
...
of any setup (84.97\,\%), suggesting that the model still separates some
canonical categories well. However, the same pattern also shows a tendency to
absorb non-standard learner productions into the nearest canonical class, so the
deficit lies in modelling the absence of canonical tone. This pattern is
consistent with a Mandarin-pretrained backbone whose
...
mis-produced by non-tonal-L1 learners. The T2/T3 contrast cited as the most
confusable pair for non-tonal-L1 learners~\cite{hao2012} appears asymmetric in
these matrices: in Setup~5, 24/125 \(T_3\) utterances are predicted as \(T_2\)
(19.2\,\%), against only 23/732 \(T_2\) utterances predicted as \(T_3\)
(3.1\,\%), so the residual confusion runs predominantly from \(T_3\) toward
\(T_2\) rather than in both directions.
...
four questions are directly tested by the experiments, while the fifth is
...
The self-supervised systems provide stronger class-balanced agreement with
the expert tone labels than the ToneNet-style CNN reference. The CNN reaches
57.32\,\% macro-F1 on its speaker-disjoint test split, while every SSL setup
reaches at least 67.33\,\% macro-F1 on the SSL speaker-disjoint split, and
the best SSL system (Setup~5) reaches 73.37\,\%. The class-level breakdown in
Table~\ref{tab:exp:class_level} shows that the gap is concentrated on the
minority categories: for \(T_1\) and \(T_2\) the CNN reaches \(F_1\) values
within a few percentage points of the SSL systems
(CNN 75.33 / 72.39\,\% vs SSL 78.29--83.39\,\%), and for \(T_4\) the CNN
trails the SSL systems by roughly 7 percentage points (CNN 78.80\,\% vs
SSL 85.66--88.07\,\%); the largest gaps appear on \(T_3\)
(CNN 21.88\,\% vs SSL 50.38--56.94\,\%) and \(T_{\mathrm{NS}}\)
(CNN 38.21\,\% vs SSL 51.42--57.53\,\%, with Setup~4 the exception at
37.13\,\%). The class-balanced advantage of SSL therefore concentrates on
\(T_3\) and \(T_{\mathrm{NS}}\) but is also visible on \(T_4\). The CNN and
SSL systems use independent speaker-disjoint splits with different test
populations, so the result is evidence that SSL representations are more
promising for this task rather than a fully controlled head-to-head
comparison.
...
Chinese Wav2Vec~2.0-Large reaches accuracy close to English-pretrained
Wav2Vec~2.0, but its weak \(T_{\mathrm{NS}}\) recall lowers macro-F1. Chinese
HuBERT-Large does not show the same failure pattern and gives the best
aggregate and class-level balance. The result therefore supports
Mandarin-specific pre-training for HuBERT in this setup, but not as a
universal explanation for improved learner-tone assessment.
...
\paragraph{RQ5: Integration with a phonetic-accuracy classifier.}
...
results show that tone prediction is strongest for \(T_1\), \(T_2\), and \(T_4\)
and weaker for \(T_3\) and \(T_{\mathrm{NS}}\). A two-dimensional CAPT feedback signal
...
The CNN baseline and the SSL systems use independent speaker-disjoint splits
with different test populations ($N=2{,}407$ vs $N=2{,}282$), so the CNN row
is treated as a reference point rather than a fully matched head-to-head
result. A directly comparable evaluation would require the image model to be
trained and tested on exactly the same speaker partition used for the SSL
systems.
