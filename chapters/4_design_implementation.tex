...
train/development/test split. The CNN baseline follows a separate
speaker-disjoint image split, matching the ToneNet-style image-classification
input format while avoiding speaker overlap between training, validation, and
test items.
...
learner corpus \cite{baevski2020}. Setup~3 follows a two-stage protocol on
the same checkpoint: \emph{Stage~A} first fine-tunes it on AISHELL-1 frame-level
Mandarin tone labels for \(T_1\)--\(T_4\), excluding neutral-tone and silence
frames from the loss, and \emph{Stage~B} then fine-tunes the resulting checkpoint
on learner speech \cite{bu2017aishell}. Setup~4 starts from
...
non-standard production, not the linguistic neutral tone. Stage~II prompts may
contain more than one syllable, but records with multi-tone expert annotations
are excluded before the experiments; every retained item therefore has one
expert tone target.
...
notation, but the AISHELL objective supervises only the four full tones
\(T_1\)--\(T_4\). Neutral-tone and silence frames are always mapped to the ignore
index, and consonant-only regions are ignored in the streams where tone
supervision is restricted to rhyme or tone-bearing phone centres. In
\texttt{label1}, by contrast, the preceding onset receives the same tone label as
the rhyme, so the whole syllable is treated as one tone-labelled region. This
stream-specific masking prevents the fifth learner output category from being
trained to mean neutral tone during native-speech adaptation. The three parallel
label streams are adapted from the design proposed by Yuan et~al.~\cite{yuan2023}:
they reuse the same waveforms and training hyperparameters but change where
...
Images are split by speaker into 9\,040 training images, 487 validation images,
and 2\,407 test images. This split is independent of the SSL partition because
the CNN branch uses all labelled recordings that can be rendered as image
features, including items without successful MFA alignments.
