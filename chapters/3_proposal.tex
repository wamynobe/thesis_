...
label per retained utterance. Records with multi-tone expert annotations are
excluded before training and evaluation, so the one-label formulation applies
only to items that have a single target tone category. The system is therefore
designed as an assessment model,
...
category \(T_{\mathrm{NS}}\). The linguistic neutral tone is not represented by
the expert label value~0 and is not modelled as a separate output class in this
work. Because the goal is to estimate performance
